...
struct Tag {
    var issSelected: Bool
    var isLocked: Bool
    var textContent: String
}

let colorUnselectedTag = UIColor.whiteColor()
let colorSelectedTag = UIColor(red:0.63, green:0.62, blue:0.89, alpha:1)

let colorTextUnSelectedTag = UIColor(red:0.48, green:0.48, blue:0.48, alpha:1)
...
class RRTagController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func createNewTag() {
        ...
        if strlen(contentTag) > 0 {
            let newTag = Tag(issSelected: false, isLocked: false, textContent: contentTag)
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.row < tags.count {
            ...
            if tags[indexPath.row].issSelected == false {
                tags[indexPath.row].issSelected = true
                selectedCell?.animateSelection(tags[indexPath.row].issSelected)
                totalTagsSelected = 1
            }
            else {
                tags[indexPath.row].issSelected = false
                selectedCell?.animateSelection(tags[indexPath.row].issSelected)
                totalTagsSelected = -1
            }
        }
    ...
    override func viewDidLoad() {
        ...
        rigthButton.tintColor = UIColor.greenColor()
        ...
        let tagController = RRTagController()
            ...
            if tagsString != nil {
                for currentTag in tagsString! {
                    tagController.tags.append(Tag(issSelected: false, isLocked: false, textContent: currentTag))
                }
            }
            tagController.blockCancel = blockCancel
            tagController.blockFinih = blockFinish
            parentController.presentViewController(tagController, animated: false, completion: nil)
    }
    ...
    class func displayTagController(#parentController: UIViewController, tags: [Tag]?,
        blockFinish: (selectedTags: Array<Tag>, unSelectedTags: Array<Tag>)->(), blockCancel: ()->()) {
            ...
            tagController.blockCancel = blockCancel
            tagController.blockFinih = blockFinish
            tagController.modalPresentationStyle = UIModalPresentationStyle.OverFullScreen
            tagController.modalTransitionStyle = UIModalTransitionStyle.CrossDissolve
